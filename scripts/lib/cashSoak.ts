...
export interface CashSoakOptions {
  ...
  bigBlind?: number
  buyIn?: number
  /** Per-opponent profiles for ai1..ai5; omitted for a homogeneous table. */
...
export function createCashSoakSession(options: CashSoakOptions = {}): CashSoakSnapshot {
  ...
  if (options.aiProfiles && options.aiProfiles.length !== CASH_SOAK_SEATS - 1) {
    throw new Error(`Cash soak needs ${CASH_SOAK_SEATS - 1} AI profiles`)
  }
  const seats = Array.from({ length: CASH_SOAK_SEATS }, (_, i) => {
    const seat: CashSoakSeat = {
      id: i === 0 ? HERO_ID : `ai${i}`,
      name: i === 0 ? 'Hero' : `AI ${i}`,
      stack: buyIn,
    }
    if (i > 0 && options.aiProfiles) seat.ai = options.aiProfiles[i - 1]
    return seat
  })
  return {
    version: 1,
...
function playHand(session: CashSoakSnapshot, profile: AiProfile): HandState {
  ...
  while (!isHandComplete(hand)) {
    ...
    const before = hand
    const actorProfile = session.seats.find((seat) => seat.id === actorId(hand))?.ai ?? profile
    const action = decideAction(hand, actorProfile, rng)
...
function actorId(hand: HandState): string {
  const actor = hand.players[hand.toActIndex]
  invariant(actor !== undefined, 'unfinished hand has no actor')
  return actor.id
}
